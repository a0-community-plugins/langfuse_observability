...
class PromptRefine(ApiHandler):
    async def process(self, input: Input, request: Request) -> Output:
        ...
        # Load refiner system prompt from plugin's prompts/ directory
        refiner_prompt_path = os.path.join(_PLUGIN_ROOT, "prompts", "prompt_refiner.md")
        try:
            refiner_prompt = files.read_file(refiner_prompt_path)
        except Exception:
